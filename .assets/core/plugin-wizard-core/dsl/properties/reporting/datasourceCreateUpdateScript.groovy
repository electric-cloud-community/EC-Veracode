 ...
 * releaseName: Name of the CloudBees CD release
 * projectName: The name of the CloudBees CD project that the release belongs to.
...
void checkRequiredParameters(Map pluginParameters) {
    def params = [
        {%- for parameter in parameters %}
        {% if parameter.required %}'{{parameter.name}}',{% endif %}
        {% endfor -%}
    ]
    params.each { it ->
...
if (!procedureExists(scheduleProjectName, scheduleName)) {
    throw EcException
            .code(ErrorCodes.ScriptError)
            .message("Procedure '${scheduleName}' does not exist in project '${scheduleProjectName}'")
            .build();
}
{% endif %}
